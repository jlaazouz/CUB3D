...
/*   Updated: 2023/10/12 20:53:53 by jlaazouz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	fill_map_data(t_data *cub)
{
	...
	cub->map_grid = (char **)ft_calloc(cub->map_len + 1, sizeof(char *));
	i = cub->map_start;
	j = 0;
	while (i < cub->file_len)
	{
		...
		{
			cub->map_grid[j] = (char *)malloc(cub->max_len + 1 * sizeof(char));
			cub->map_grid[j][cub->max_len] = 0;
			ft_memset(cub->map_grid[j], ' ', cub->max_len);
